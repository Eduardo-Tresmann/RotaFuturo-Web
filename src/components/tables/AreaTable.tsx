...
export function AreaTable({ areas, onEdit, onInativar }: AreaTableProps) {
  ...
  const filtered = useMemo(() => {
    let data = areas.filter((a: Area) => {
      const matchSearch =
        a.areaDescricao.toLowerCase().includes(search.toLowerCase()) ||
        String(a.areaId).includes(search);
      const matchId = filterId ? String(a.areaId) === filterId : true;
      const matchDescricao = filterDescricao
        ? a.areaDescricao.toLowerCase().includes(filterDescricao.toLowerCase())
        : true;
      const matchAtivo = filterAtivo ? String(a.areaAtivo) === filterAtivo : true;
    ...
    return data;
  }, [areas, search, sortKey, sortAsc, filterId, filterDescricao, filterAtivo]);
  ...
  return (
    <div>
      ...
      <Table>
        <TableCaption>Lista de áreas do sistema</TableCaption>
        <TableHeader>
          <TableRow>
            <TableHead className="cursor-pointer" onClick={() => handleSort('areaId')}>
              ID {sortKey === 'areaId' && (sortAsc ? '▲' : '▼')}
            </TableHead>
            <TableHead className="cursor-pointer" onClick={() => handleSort('areaDescricao')}>
              Descrição {sortKey === 'areaDescricao' && (sortAsc ? '▲' : '▼')}
            </TableHead>
            <TableHead className="cursor-pointer" onClick={() => handleSort('areaAtivo')}>
              Ativo {sortKey === 'areaAtivo' && (sortAsc ? '▲' : '▼')}
            </TableHead>
          ...
          {paginated.length === 0 ? (
            ...
            paginated.map((area) => (
              <TableRow key={area.areaId}>
                ...
                <TableCell>{area.areaDescricao}</TableCell>
                <TableCell>
                  <span
                    className={
                      area.areaAtivo ? 'text-green-600 font-bold' : 'text-red-600 font-bold'
                    }
                  >
                    {area.areaAtivo ? 'Ativo' : 'Inativo'}
                  </span>
                </TableCell>
                <TableCell>
                  <button
